...
parser.add_argument(
    "-r", "--rerun", help="refresh the post installation procedure", action="store_true"
)
...
def github_create_repo() -> None:
    ...
    body_json = {
        "name": "pynnacle",
        "description": "A utility class to simplify sending emails from within python packages.",
    }

    url = "https://api.github.com/user/repos"
    header = {"Authorization": f"token {GITHUB_TOKEN}"}
    response = requests.post(url, json=body_json, headers=header, timeout=5)
...
def github_create_secret(secret_name: str, secret_value: str) -> None:
    ...
    url_public_key = (
        ...
        "/pynnacle/actions/secrets/public-key"
    )
    ...
    if r.status_code == 200:
        ...
        url_secret = (
            ...
            f"Stephen-RA-King"
            f"/pynnacle/actions/secrets/{secret_name}"
        )
...
def readthedocs_create() -> None:
    ...
    body_json = {
        ...
        "homepage": "http://pynnacle.readthedocs.io/",
    ...
    response = requests.post(url, json=body_json, headers=header, timeout=10)
    if response.status_code == 201:
        logger.info(".... OK")
...
def readthedocs_update() -> None:
    ...
    url = "https://readthedocs.org/api/v3/projects/pynnacle/"
    header = {"Authorization": f"token {READTHEDOCS_TOKEN}"}
    response = requests.patch(url, json=body_json, headers=header, timeout=5)
